...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  MarsResponse PublicInterface(MarsRequest req, Trace trace)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := mars.NewMarsServiceClientFactory(trans, protocolFactory)
  ...
  case "PublicInterface":
    if flag.NArg() - 1 != 2 {
      fmt.Fprintln(os.Stderr, "PublicInterface requires 2 args")
      flag.Usage()
    }
    arg4 := flag.Arg(1)
    ...
    value1 := argvalue1
    fmt.Print(client.PublicInterface(value0, value1))
    fmt.Print("\n")
    break
  case "":
    Usage()
